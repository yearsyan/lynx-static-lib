...
```console
python scripts/package_conan.py --skip-build --version 0.2
```
...
```console
python scripts/package_conan.py --skip-build --upload --version 0.2 --remote neuyan
```
...
```console
python scripts/package_http_conan.py --version 0.2
```

The script uses `conancenter` for public dependencies by default and expects
`lynxlib/0.2@neuyan/stable` to already exist in the local Conan cache.
...
```console
python scripts/package_http_conan.py --version 0.2 --upload --remote neuyan
```
...
```json
"conan": {
  "lynxlib": "lynxlib/0.2@neuyan/stable",
  "runtime": "lynxlib-runtime/0.2@neuyan/stable",
  "http": "lynxlib-http/0.2@neuyan/stable"
}
```
